...
SMALL_BLOCK_URDF = "models/drake/objects/block_for_pick_and_place_small_size.urdf"
BLOCK_URDF = "models/drake/objects/block_for_pick_and_place_mid_size.urdf"
LARGE_BLOCK_URDF = "models/drake/objects/block_for_pick_and_place_large_size.urdf"
...
def load_pybullet(filename, client, fixed_base=False, scale=1., **kwargs):
    # fixed_base=False implies infinite base mass
    with LockRenderer(client):
...
def load_model(rel_path, pose=None, **kwargs):
    ...
    add_data_path()
    #with LockRenderer():
    body = load_pybullet(abs_path, client, **kwargs)
    if pose is not None:
        set_pose(body, pose)
    return body
...
def set_renderer(client, enable):
...
class LockRenderer(Saver):
    ...
    def __init__(self, client, lock=True):
        # skip if the visualizer isn't active
        self.client = client
        self.renderer = has_gui(self.client)
        if lock:
            set_renderer(self.client, enable=False)

    def restore(self):
        set_renderer(self.client, enable=self.renderer)
...
def set_camera(client, yaw, pitch, distance, target_position=np.zeros(3)):
...
def image_from_segmented(segmented, color_from_body=None):
    if color_from_body is None:
        bodies = get_bodies()
        color_from_body = dict(list(zip(bodies, spaced_colors(len(bodies)))))
...
def set_default_camera(client):
    set_camera(client, 160, -35, 2.5, Point())
...
def Pose(point=None, euler=None):
    ...
    return (point, quat_from_euler(euler, client))

#def Pose2d(x=0., y=0., yaw=0.):
#    return np.array([x, y, yaw])

def invert(pose, client):
    (point, quat) = pose
    return p.invertTransform(point, quat, physicsClientId=client)

def multiply(*poses, client):
    pose = poses[0]
    for next_pose in poses[1:]:
        pose = p.multiplyTransforms(pose[0], pose[1], *next_pose, physicsClientId=client)
    return pose
...
def quat_from_euler(euler, client):
    return p.getQuaternionFromEuler(euler, physicsClientId=client)

def euler_from_quat(quat, client):
    return p.getEulerFromQuaternion(quat, physicsClientId=client)

def unit_point():
    return (0., 0., 0.)

def unit_quat(client):
    return quat_from_euler([0, 0, 0], client) # [X,Y,Z,W]
...
def unit_pose(client):
    return (unit_point(), unit_quat(client))
...
def get_point(body, client):
    return get_pose(body, client)[0]
...
def dump_body(body):
    print('Body id: {} | Name: {} | Rigid: {} | Fixed: {} | Pose: {} '.format(
        body, get_body_name(body), is_rigid_body(body), is_fixed_base(body), get_pose(body)))
    # for joint in get_joints(body):
    #     if is_movable(body, joint):
...
def dump_body_yd(body, client):
    link = -1
    mobile = is_fixed_base(body)
    pos_info = get_pose(body)
    vis_info = get_visual_data(body, link)[0]
    body_info = {}
    body_info['color'] = np.array(vis_info.rgbaColor)
    body_info['size'] = np.array(vis_info.dimensions)
    body_info['pos'] = np.array(pos_info[0])
    body_info['orientation'] = p.getEulerFromQuaternion(np.array(pos_info[1]), physicsClientId=client)
    body_info['fixed'] = copy.copy(mobile)
...
def dump_world_yd(obj_names):

    obj_state = {}
    for obj_name, id in list(obj_names.items()):
        obj_state[obj_name] = dump_body_yd(id)
...
def get_labeled_configuration(body):
    ...
    return dict(list(zip(get_joint_names(body, movable_joints),
...
def get_all_link_children(body):
    children = {}
    for child, parent in list(get_all_link_parents(body).items()):
...
def get_adjacent_fixed_links(body):
    return list([item for item in get_adjacent_links(body) if not is_movable(body, item[0])])
...
def get_dynamics_info(body, link=BASE_LINK):
    return DynamicsInfo(*p.getDynamicsInfo(body, link, physicsClientId=client)[:10])
...
def get_mesh_geometry(path, scale=1.):
    ...
    # if scale != 1.0:
    #     mesh_dict['meshScale'][1] = 0.5

    return mesh_dict

NULL_ID = -1

def create_shape(geometry, client, pose=None, color=(1, 0, 0, 1), specular=None):
    if pose is None:
        pose = unit_pose(client)
    point, quat = pose
    collision_args = {
    ...
    collision_id = p.createCollisionShape(**collision_args, physicsClientId=client)
...
def create_shape_array(geoms, poses, colors=None):
    ...
    for geom in geoms:
        ...
        for key, value in list(extended_geom.items()):
...
def get_visual_data(body, client, link=BASE_LINK):
    visual_data = [VisualShapeData(*tup) for tup in p.getVisualShapeData(body, physicsClientId=client)]
    return list([d for d in visual_data if d.linkIndex == link])
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    for joint, value in zip(list(range(len(links))), get_joint_positions(body, links)):
...
def get_subtree_aabb(body, root_link=BASE_LINK):
    return aabb_union(get_aabb(body, client, link) for link in get_link_subtree(body, root_link))

def get_aabbs(body, client):
...
def get_aabb(body, link=None):
    ...
    if link is None:
        aabb = aabb_union(get_aabbs(body, client))
    else:
...
def get_center_extent(body, client, **kwargs):
    aabb = get_aabb(body, client, **kwargs)
...
def approximate_as_prism(body, client, body_pose=None): #, **kwargs):
    if body_pose is None:
        body_pose = unit_pose(client)
...
def in_collision(client):
    bodies = get_bodies(client)
    for i in range(len(bodies)):
        for j in range(0, i):
            collision = pairwise_collision(client, bodies[i], bodies[j])
            if collision:
                return True
...
def get_self_link_pairs(body, joints, disabled_collisions=set(), only_moving=True):
    ...
    check_link_pairs = list([pair for pair in check_link_pairs if not are_links_adjacent(body, *pair)])
    check_link_pairs = list([pair for pair in check_link_pairs if (pair not in disabled_collisions) and
...
def stable_z_on_aabb(body, aabb, client):
    center, extent = get_center_extent(body, client)
    _, upper = aabb
    return (upper + extent/2 + (get_point(body, client) - center))[2]
...
def sample_placement_on_aabb(client, top_body, bottom_aabb, top_pose=None,
                             percent=1.0, max_attempts=50, epsilon=1e-3):
    if top_pose is None:
        top_pose = unit_pose(client)
...
def get_quaternion_waypoints(point, start_quat, end_quat, step_size=np.pi/16):
    ...
    for t in np.arange(0, angle, step_size):
        fraction = t/angle
        quat = p.getQuaternionSlerp(start_quat, end_quat, interpolationFraction=fraction, physicsClientId=client)
...
def interpolate_poses(pose1, pose2, pos_step_size=0.01, ori_step_size=np.pi/16):
    ...
    for i in range(num_steps):
        ...
        quat = p.getQuaternionSlerp(quat1, quat2, interpolationFraction=fraction, physicsClientId=client)
...
def get_aabb_vertices(aabb):
    ...
    return [tuple(aabb[i[k]][k] for k in range(d))
            for i in product(list(range(len(aabb))), repeat=d)]

def draw_aabb(aabb, **kwargs):
    d = len(aabb[0])
    vertices = list(product(list(range(len(aabb))), repeat=d))
    lines = []
